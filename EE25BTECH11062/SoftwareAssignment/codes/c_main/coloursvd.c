...
void process_data(FILE *fptr, int w, int h, int max, int **r_data, int **g_data, int **b_data, MAT *r_out, MAT *b_out, MAT *g_out){
    ...
    for(int i = 0; i<h; i++){
        for(int j = 0; j<w; j++){
            fread(*(r_data+i)+j, bitsize, 1, fptr);
            r_out->data[w*i+j] = *(*(r_data+i)+j);
            fread(*(g_data+i)+j, bitsize, 1, fptr);
            g_out->data[w*i+j] = *(*(g_data+i)+j);
            fread(*(b_data+i)+j, bitsize, 1, fptr);
            b_out->data[w*i+j] = *(*(b_data+i)+j);
        }
    }
}
...
int main(){
    ...
    if(fptr == NULL)
        ...
        int **r_data = (int **)calloc(h, sizeof(int *));
        int **g_data = (int **)calloc(h, sizeof(int *));
        int **b_data = (int **)calloc(h, sizeof(int *));
        for(int i = 0; i<h; i++){
            *(r_data+i) = (int *)calloc(w, sizeof(int));
            *(g_data+i) = (int *)calloc(w, sizeof(int));
            *(b_data+i) = (int *)calloc(w, sizeof(int));
        }
        MAT r_out, r_res, r_restricted_res, r_diff;
        MAT g_out, g_res, g_restricted_res, g_diff;
        MAT b_out, b_res, b_restricted_res, b_diff;
        process_data(fptr, w, h, max, r_data, g_data, b_data, &r_out, &g_out, &b_out);
        matInit(&r_res, r_out.rows, r_out.cols);
        matInit(&g_res, g_out.rows, g_out.cols);
        matInit(&b_res, b_out.rows, b_out.cols);
        printf("Enter a k value between 1 and %d:\n", (w-h>0)?h:w);
        scanf("%d", &k);
        svd(&r_out, k, &r_res);
        svd(&g_out, k, &g_res);
        svd(&b_out, k, &b_res);
        matInit(&r_restricted_res, r_res.rows, r_res.cols);
        matInit(&g_restricted_res, g_res.rows, g_res.cols);
        matInit(&b_restricted_res, b_res.rows, b_res.cols);
        for(int i = 0; i<h; i++){
            for(int j = 0; j<w; j++){
                int r = (int)r_res.data[w*i+j];
                r = (r>0)?r:0;
                r = (r<max)?r:max;
                r_restricted_res.data[w*i+j] = r;
                int g = (int)r_res.data[w*i+j];
                g = (g>0)?g:0;
                g = (g<max)?g:max;
                g_restricted_res.data[w*i+j] = g;
                int b = (int)r_res.data[w*i+j];
                b = (b>0)?b:0;
                b = (b<max)?b:max;
                b_restricted_res.data[w*i+j] = b;
            }
        }
        sub(&r_out, &r_res, &r_diff);
        sub(&g_out, &g_res, &g_diff);
        sub(&b_out, &b_res, &b_diff);
        float r_frobenius_norm = norm(&r_diff);
        float g_frobenius_norm = norm(&g_diff);
        float b_frobenius_norm = norm(&b_diff);
        printf("Frobenius norm ||A-A_k|| of r channel = %f\n", r_frobenius_norm);
        printf("Frobenius norm ||A-A_k|| of g channel = %f\n", g_frobenius_norm);
        printf("Frobenius norm ||A-A_k|| of b channel = %f\n", b_frobenius_norm);
        fclose(fptr);
        FILE *fptr2 = fopen(output_name, "wb+");
        fprintf(fptr2, "P6 %d %d %d\n", w, h, max);
        int bitsize = (max<256) ? 1 : 2;
        for(int i = 0; i<h; i++){
            for(int j = 0; j<w; j++){
                int r = (int)r_restricted_res.data[w*i+j];
                int g = (int)g_restricted_res.data[w*i+j];
                int b = (int)b_restricted_res.data[w*i+j];
                fwrite(&r, bitsize, 1, fptr2);
                fwrite(&g, bitsize, 1, fptr2);
                fwrite(&b, bitsize, 1, fptr2);
            }
        }
        //char conv_cmd_2[300];
        ...
        matFree(&r_out);
        matFree(&r_res);
        matFree(&r_restricted_res);
        matFree(&r_diff);
        free_data(r_data, h);
        matFree(&g_out);
        matFree(&g_res);
        matFree(&g_restricted_res);
        matFree(&g_diff);
        free_data(g_data, h);
        matFree(&b_out);
        matFree(&b_res);
        matFree(&b_restricted_res);
        matFree(&b_diff);
        free_data(b_data, h);
        fclose(fptr2);
    }
